...
export default function Projects() {
    ...
    const {projectsOverview, fetchPortfolio} = usePortfolio();
    // ---- Modal states ----
    ...
    return (
        <>
            <section className="mb-16 md:mb-0 space-y-10">
                <h1 className="text-3xl font-bold">Manage Your Projects Sections</h1>
                {/* Refresh button */}
                <Button onClick={() => fetchPortfolio()}>Refresh <FiRotateCcw className="inline" /></Button>

                { projectsOverview && 
                    <Card key={projectsOverview._id} className="p-4 md:p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-semibold">{projectsOverview.title}</h2>
                            <Button onClick={() => { setFormData({ title: projectsOverview.title, description: projectsOverview.description, isActive: projectsOverview.isActive }); setEditingOverview(projectsOverview); }}>
                                Edit Section
                            </Button>
                        </div>
                        <p className="text-gray-600">{projectsOverview.description}</p>

                        {/* Section Buttons */}
                        <div>
                            <h3 className="font-medium mb-2">Section Buttons</h3>
                            {projectsOverview.buttons?.length > 0 ? projectsOverview.buttons.map((btn) => (
                                <div key={btn._id} className="flex justify-between items-center bg-gray-50 p-2 rounded mb-2">
                                    ...
                                    <div className="space-x-2">
                                        <Button size="sm" onClick={() => { setButtonData({ text: btn.text, link: btn.link, buttonIcon: btn.buttonIcon }); setEditingButton({ ...btn, overviewId: projectsOverview._id, type: "overview" }); }}><Edit/></Button>
                            ...
                            <Button size="sm" className="mt-2" onClick={() => { setButtonData({ text: "", link: "", buttonIcon: "" }); setAddingButtonOverview(projectsOverview); }}>+ Add Section Button</Button>
                              <Button size="sm" className="mt-2" onClick={() => { setCardData({ title: "", description: "", imgUrl: "", icon: "" }); setAddingCardOverview(projectsOverview); }}>+ Add Project</Button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {projectsOverview.cards?.length > 0 ? projectsOverview.cards.map((card) => (
                                <Card key={card._id} className="p-4 mb-2">
                                    <h4 className="font-semibold">{card.title}</h4>
                                      <img src={card.imgUrl} alt={card.title} className="mb-2 h-56 md:w-64 rounded-t-lg" />
                                    <p className="text-gray-600">{card.description}</p>
                                    <div className="mt-2 space-x-2">
                                        <Button size="sm" onClick={() => { setCardData({ title: card.title, description: card.description, imgUrl: card.imgUrl, tags: card.tags, icon: card.icon }); setEditingCard({ ...card, overviewId: projectsOverview._id }); }}><Edit/></Button>
                                        <Button size="sm" variant="destructive" onClick={() => handleDeleteCard(card._id)}><Trash/></Button>
                                    </div>

                                    {/* Buttons inside Card */}
                                    <div className="mt-2">
                                        {card.buttons?.map((btn) => (
                                            <div key={btn._id} className="flex flex-wrap space-y-1 justify-between items-center bg-gray-100 p-2 rounded mb-1">
                                                ...
                                                <div className="space-x-2">
                                                    ...
                                                    <Button size="sm" variant="destructive" onClick={() => handleDeleteButton(btn._id)}><Trash/></Button>
                                                </div>
                                            </div>
                                        ))}
                ...
                <Dialog open={!!editingOverview} onOpenChange={() => setEditingOverview(null)}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Edit Project Section</DialogTitle>
                        </DialogHeader>
