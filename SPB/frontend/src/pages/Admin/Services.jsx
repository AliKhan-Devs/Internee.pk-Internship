...
export default function Services() {
    ...
    const { servicesOverview, fetchPortfolio } = usePortfolio();
    // ---- Modal states ----
    ...
    return (
        <>
            <section className="mb-16 md:mb-0 space-y-10">
                <h1 className="text-3xl font-bold">Manage Your Services Sections</h1>
                {/* Refresh button */}
                <Button onClick={() => fetchPortfolio()}>Refresh <FiRotateCcw className="inline" /></Button>

                {servicesOverview &&
                    <Card key={servicesOverview._id} className="p-4 md:p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-semibold">{servicesOverview.title}</h2>
                            <Button onClick={() => { setFormData({ title: servicesOverview.title, description: servicesOverview.description, isActive: servicesOverview.isActive }); setEditingOverview(servicesOverview); }}>
                                Edit Section
                            </Button>
                        </div>
                        <p className="text-gray-600">{servicesOverview.description}</p>

                        {/* Section Buttons */}
                        <div>
                            <h3 className="font-medium mb-2">Section Buttons</h3>
                            {servicesOverview.buttons?.length > 0 ? servicesOverview.buttons.map((btn) => (
                                <div key={btn._id} className="flex justify-between items-center bg-gray-50 p-2 rounded mb-2">
                                    ...
                                    <div className="space-x-2">
                                        <Button size="sm" onClick={() => { setButtonData({ text: btn.text, link: btn.link, buttonIcon: btn.buttonIcon }); setEditingButton({ ...btn, overviewId: servicesOverview._id, type: "overview" }); }}><Edit/></Button>
                            ...
                            <div className="flex gap-4 flex-wrap">
                                ...
                                <Button size="sm" className="mt-2" onClick={() => { setButtonData({ text: "", link: "", buttonIcon: "" }); setAddingButtonOverview(servicesOverview); }}>+ Add Section Button</Button>
                                <Button size="sm" className="mt-2" onClick={() => { setCardData({ title: "", description: "", imgUrl: "", icon: "" }); setAddingCardOverview(servicesOverview); }}>+ Add Service</Button>
                            </div>
                        </div>

                        {/* Cards */}
                        <div>
                            <h3 className="font-medium mb-2">Your Services</h3>
                            {servicesOverview.cards?.length > 0 ? servicesOverview.cards.map((card) => (
                                <Card key={card._id} className="p-4 mb-2">
                                    ...
                                    <div className="mt-2 space-x-2">
                                        <Button size="sm" onClick={() => { setCardData({ title: card.title, description: card.description, imgUrl: card.imgUrl, tags: card.tags, icon: card.icon }); setEditingCard({ ...card, overviewId: servicesOverview._id }); }}><Edit/></Button>
