...
export default function Stats() {
    ...
    const { statsOverview, fetchPortfolio } = usePortfolio();
    // ---- Modal states ----
    ...
    return (
        <>
            <section className="mb-16 md:mb-0 space-y-10">
                <h1 className="text-3xl font-bold">Manage Your Quick Stats Sections</h1>
                {/* Refresh button */}
                <Button onClick={() => fetchPortfolio()}>Refresh <FiRotateCcw className="inline" /></Button>

                {statsOverview &&
                    <Card key={statsOverview._id} className="p-4 md:p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-semibold">{statsOverview.title}</h2>
                            <Button onClick={() => { setFormData({ title: statsOverview.title, description: statsOverview.description, isActive: statsOverview.isActive }); setEditingOverview(statsOverview); }}>
                                Edit Section
                            </Button>
                        </div>
                        <p className="text-gray-600">{statsOverview.description}</p>

                        {/* Section Buttons */}
                        <div>
                            <h3 className="font-medium mb-2">Section Buttons</h3>
                            {statsOverview.buttons?.length > 0 ? statsOverview.buttons.map((btn) => (
                                <div key={btn._id} className="flex justify-between items-center bg-gray-50 p-2 rounded mb-2">
                                    ...
                                    <div className="space-x-2">
                                        <Button size="sm" onClick={() => { setButtonData({ text: btn.text, link: btn.link, buttonIcon: btn.buttonIcon }); setEditingButton({ ...btn, overviewId: statsOverview._id, type: "overview" }); }}><Edit/></Button>
                            ...
                            <Button size="sm" className="mt-2" onClick={() => { setButtonData({ text: "", link: "", buttonIcon: "" }); setAddingButtonOverview(statsOverview); }}>+ Add Section Button</Button>
                            <Button size="sm" className="mt-2" onClick={() => { setCardData({ title: "", description: "", imgUrl: "", icon: "" }); setAddingCardOverview(statsOverview); }}>+ Add New Stat</Button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex justify-center flex-wrap gap-4 items-center items-stretch">

                                {statsOverview.cards?.length > 0 ? statsOverview.cards.map((card) => (
                                    <Card key={card._id} className="p-4 mb-2 min-w-[280px] flex-1">
                                        <RenderIcon iconName={card.icon} size={20} color={'black'} className={"inline"} />
                                        <h4 className="font-semibold">{card.title}</h4>
                                        <p className="text-gray-600">{card.description}</p>
                                        <div className="mt-2 space-x-2">
                                            <Button size="sm" onClick={() => { setCardData({ title: card.title, description: card.description, imgUrl: card.imgUrl, tags: card.tags, icon: card.icon }); setEditingCard({ ...card, overviewId: statsOverview._id }); }}><Edit/></Button>
