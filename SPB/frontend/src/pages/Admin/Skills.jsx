...
export default function Skills() {
    ...
    const { skillsOverview, fetchPortfolio } = usePortfolio();
    // ---- Modal states ----
    ...
    return (
        <>
            <section className="mb-16 md:mb-0 space-y-10">
                <h1 className="text-3xl font-bold">Manage Your Skills Sections</h1>
                {/* Refresh button */}
                <Button onClick={() => fetchPortfolio()}>Refresh <FiRotateCcw className="inline" /></Button>

                {skillsOverview &&
                    <Card key={skillsOverview._id} className="p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-semibold">{skillsOverview.title}</h2>
                            <Button onClick={() => { setFormData({ title: skillsOverview.title, description: skillsOverview.description, isActive: skillsOverview.isActive }); setEditingOverview(skillsOverview); }}>
                                Edit Section
                            </Button>
                        </div>
                        <p className="text-gray-600">{skillsOverview.description}</p>

                        {/* Section Buttons */}
                        <div>
                            <h3 className="font-medium mb-2">Section Buttons</h3>
                            {skillsOverview.buttons?.length > 0 ? skillsOverview.buttons.map((btn) => (
                                <div key={btn._id} className="flex justify-between items-center bg-gray-50 p-2 rounded mb-2">
                                    ...
                                    <div className="space-x-2">
                                        <Button size="sm" onClick={() => { setButtonData({ text: btn.text, link: btn.link, buttonIcon: btn.buttonIcon }); setEditingButton({ ...btn, overviewId: skillsOverview._id, type: "overview" }); }}>Edit</Button>
                            ...
                            <Button size="sm" className="mt-2" onClick={() => { setButtonData({ text: "", link: "", buttonIcon: "" }); setAddingButtonOverview(skillsOverview); }}>+ Add Section Button</Button>
                            <Button size="sm" className="mt-2" onClick={() => { setCardData({ title: "", description: "", imgUrl: "", icon: "" }); setAddingCardOverview(skillsOverview); }}>+ Add Skill</Button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-6 md:gap-12 items-center justify-center">
                                {skillsOverview.cards?.length > 0 ? skillsOverview.cards.map((card) => (
                                    <Card key={card._id} className="p-4 mb-2">
                                        <RenderIcon iconName={card.icon} size={20} color={'black'} className={"inline"} />
                                        <h4>{card.title}</h4>
                                        <div className="mt-2 space-x-2">
                                            <Button size="sm" onClick={() => { setCardData({ title: card.title, description: card.description, imgUrl: card.imgUrl, tags: card.tags, icon: card.icon }); setEditingCard({ ...card, overviewId: skillsOverview._id }); }}><Edit/></Button>
