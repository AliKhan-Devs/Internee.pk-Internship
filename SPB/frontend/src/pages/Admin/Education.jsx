...
export default function Education() {
    ...
    const {educationOverview, fetchPortfolio} = usePortfolio();
    // ---- Modal states ----
    ...
    const handleSaveOverview = async () => {
        ...
        try {
            ...
            toast.error("Failed to update overview", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <>
            <section className="mb-16 md:mb-0 space-y-10">
                <h1 className="text-3xl font-bold">Manage Your Educaion Sections</h1>
                {/* Refresh button */}
                <Button onClick={() => fetchPortfolio()}>Refresh <FiRotateCcw className="inline" /></Button>

                { educationOverview && 
                    <Card key={educationOverview._id} className="p-4 md:p-6 space-y-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-2xl font-semibold">{educationOverview.title}</h2>
                            <Button onClick={() => { setFormData({ title: educationOverview.title, description: educationOverview.description, isActive: educationOverview.isActive }); setEditingOverview(educationOverview); }}>
                                Edit Section
                            </Button>
                        </div>
                        <p className="text-gray-600">{educationOverview.description}</p>

                        {/* Section Buttons */}
                        <div>
                            <h3 className="font-medium mb-2">Section Buttons</h3>
                            {educationOverview.buttons?.length > 0 ? educationOverview.buttons.map((btn) => (
                                <div key={btn._id} className="flex justify-between items-center bg-gray-50 p-2 rounded mb-2">
                                    ...
                                    <div className="space-x-2">
                                        <Button size="sm" onClick={() => { setButtonData({ text: btn.text, link: btn.link, buttonIcon: btn.buttonIcon }); setEditingButton({ ...btn, overviewId: educationOverview._id, type: "overview" }); }}>Edit</Button>
                            ...
                            <Button size="sm" className="mt-2" onClick={() => { setButtonData({ text: "", link: "", buttonIcon: "" }); setAddingButtonOverview(educationOverview); }}>+ Add Section Button</Button>
                             <Button size="sm" className="mt-2" onClick={() => { setCardData({ title: "", description: "", imgUrl: "", icon: "" }); setAddingCardOverview(educationOverview); }}>+ Add Education</Button>
                            </div>
                        </div>

                        {/* Cards */}
                        <div>
                            <h3 className="font-medium mb-2">Your Education</h3>
                            {educationOverview.cards?.length > 0 ? educationOverview.cards.map((card) => (
                                <Card key={card._id} className="p-4 mb-2">
                                    ...
                                    <div className="mt-2 space-x-2">
                                        <Button size="sm" onClick={() => { setCardData({ title: card.title, description: card.description, imgUrl: card.imgUrl, tags: card.tags, icon: card.icon }); setEditingCard({ ...card, overviewId: educationOverview._id }); }}><Edit/></Button>
